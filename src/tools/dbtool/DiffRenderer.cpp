...
namespace
{
                           ...
                           std::function<tui::Style const*(std::size_t, std::size_t)> cellStyle)
    {
        ...
        auto const drawRow = [&](std::vector<std::string> const& cells,
            ...
            for (auto const& [col, w]: std::views::enumerate(widths))
            {
                ...
                auto const text = std::cmp_less(col, cells.size())
                    ? tui::truncateToDisplayWidth(cells[colSize], w)
                    : std::string {};
                auto const alignment = std::cmp_less(col, table.alignments.size())
                    ? table.alignments[colSize]
    ...
    [[nodiscard]] std::string DescribeColumnDiff(ColumnDiff const& cd)
    {
        ...
        {
            case DiffKind::OnlyInA:
                return cd.a ? std::format("only in A: {}", cd.a->dialectDependantTypeString) : "only in A";
            case DiffKind::OnlyInB:
                return cd.b ? std::format("only in B: {}", cd.b->dialectDependantTypeString) : "only in B";
            case DiffKind::Changed: {
                ...
                if (cd.a && cd.b)
                    details += std::format(" ({} → {})", cd.a->dialectDependantTypeString,
                                            cd.b->dialectDependantTypeString);
                return details;
            }
        }
        return {};
    ...
    void RenderSchemaDiff(std::ostream& out,
                          SchemaDiff const& diff,
                          Palette const& palette,
                          bool useColor,
                          int maxWidth)
    {
        ...
        {
            auto const qualifiedName = td.schema.empty() ? td.name : std::format("{}.{}", td.schema, td.name);
            pt.rows.push_back({ qualifiedName, DiffKindLabel(td.kind), DescribeTableDiff(td) });
        ...
        EmitBorderedTable(
            out, pt, palette, useColor, maxWidth, [&](std::size_t r, std::size_t /*c*/) -> tui::Style const* {
    ...
    void RenderTableDataDiff(std::ostream& out,
                             TableDataDiff const& d,
                             Palette const& palette,
                             bool useColor,
                             int maxWidth)
    {
        EmitSectionHeader(
            ...
            std::format("Data diff: {} (A: {} rows, B: {} rows{})",
                        d.tableName, d.aRowCount, d.bRowCount, d.truncated ? ", truncated" : ""),
            palette, useColor);
        ...
        pt.alignments = { tui::TableAlignment::Left, tui::TableAlignment::Left, tui::TableAlignment::Left,
                          tui::TableAlignment::Left, tui::TableAlignment::Left };
        pt.columnCount = 5;
