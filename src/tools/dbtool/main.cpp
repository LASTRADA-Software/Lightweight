...
[[nodiscard]] Tools::PluginIngestOptions MakeIngestOptions(bool verbose)
{
    ...
    if (verbose)
        opts.logShadowed = [](std::string_view msg) { std::println(std::cerr, "{}", msg); };
    opts.logLoading = [](std::string_view msg) { std::println("{}", msg); };
    opts.logError = [](std::string_view msg) { std::println(std::cerr, "{}", msg); };
...
int ListProfiles(Options const& options)
{
    ...
    for (auto const& profile: store.Profiles())
    {
        ...
        // Profiles whose effective plugin search uses the multi-directory
        // YAML form collapse into a single column here. The list separator
        // mirrors $PATH conventions for the platform so users recognise it.
        ...
        constexpr std::string_view PathSep =
            ...
            ":";
#endif
        for (auto const& [idx, dir]: std::views::enumerate(effectiveDirs))
        {
            if (idx > 0)
                joinedDirs += PathSep;
            joinedDirs += dir.string();
        }
        ...
        rows.push_back(Row {
            ...
            .pluginsDir = std::move(joinedDirs),
        });
    }
