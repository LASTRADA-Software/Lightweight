...
class SqlSyntaxHighlighter: public QSyntaxHighlighter
{
  ...
  private:
    ...
    /// the document sizes a query editor sees. `captureIndex` selects which
    /// capture group of the match to format — defaults to 0 (the whole
    /// match); use a positive index to colour only a sub-group (e.g. to
    /// highlight an identifier that follows a contextual keyword without
    /// re-colouring the keyword itself).
