...
namespace
{
    ...
    [[nodiscard]] Lightweight::Tools::PluginIngestOptions MakeGuiIngestOptions(bool verbose)
    {
        ...
        if (verbose)
        {
            opts.logShadowed = [](std::string_view msg) { qInfo("%s", std::string(msg).c_str()); };
            opts.logLoading = [](std::string_view msg) { qInfo("%s", std::string(msg).c_str()); };
        }
        opts.logError = [](std::string_view msg) { qWarning("%s", std::string(msg).c_str()); };
