...
void SqlQueryRunner::execute(QString const& sql)
{
    ...
    // Reset the stop source so a stale `cancel()` from a prior run cannot
    // race-cancel this fresh query. The token captured below sees the new
    // shared-state.
    _stopSource = std::stop_source {};
    auto const stopToken = _stopSource.get_token();
    ...
    _pool.start(new FunctionTask([runner, model, queryStd, stopToken] {
        QElapsedTimer timer;
        timer.start();
        try
        {
            ...
            // Publish the native handle so `cancel()` (GUI thread) can call
            // `SQLCancel` against it. Cleared in every exit path below.
            runner->_activeStmt.store(stmt.NativeHandle(), std::memory_order_release);
            ...
            if (numColumns == 0)
            {
                ...
                runner->_activeStmt.store(nullptr, std::memory_order_release);
            ...
            QString status;
            if (cancelled)
                status = QStringLiteral("cancelled after %1 row(s) in %2 ms").arg(rowCount).arg(elapsedMs);
            else if (truncated)
                status = QStringLiteral("%1 row(s) (truncated) in %2 ms").arg(rowCount).arg(elapsedMs);
            else
                status = QStringLiteral("%1 row(s) in %2 ms").arg(rowCount).arg(elapsedMs);
            PostFinished(runner, rowCount, elapsedMs, status);
        }
        catch (Lightweight::SqlException const& ex)
        {
            // A successful `SQLCancel` surfaces as a driver-specific
            // SQLSTATE here (e.g. HY008 "Operation canceled"). Normalise it
            // to a friendly message rather than letting "Operation
            // canceled" slip through as a hard error.
            auto const msg =
                stopToken.stop_requested() ? QStringLiteral("query cancelled") : QString::fromStdString(ex.info().message);
...
void SqlQueryRunner::cancel()
{
    if (!busy())
        return;
    _stopSource.request_stop();
    // `SQLCancel` is documented as safe to call from another thread on a
    // statement handle that is currently executing — that is, in fact, its
    // primary use case. Acquire pairs with the worker's release store of
    // the handle.
    auto* const native = _activeStmt.load(std::memory_order_acquire);
    if (native != nullptr)
        SQLCancel(static_cast<SQLHSTMT>(native));
}
