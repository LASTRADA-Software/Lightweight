...
namespace
{
    ...
    [[nodiscard]] FilenameKey FilenameKeyFromPath(std::filesystem::path const& p)
    {
        ...
        std::ranges::transform(key, key.begin(), [](wchar_t c) {
                                            ...
                                            std::function<void(std::string_view)> const& logShadowed)
{
    ...
    for (auto const [dirIndex, dir]: std::views::enumerate(dirs))
        ScanOneDirectory(dir, static_cast<std::size_t>(dirIndex), encounter, winners, logShadowed);
