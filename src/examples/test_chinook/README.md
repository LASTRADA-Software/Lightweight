...
``` sh
sqlcmd -S localhost -U sa -P "BlahThat." -C -d LightweightTest -i ./src/examples/test_chinook/Chinook_Sqlite.sql
```
...
``` sh
./build/src/tools/ddl2cpp --connection-string "DRIVER={ODBC Driver 18 for SQL Server};SERVER=localhost;UID=sa;PWD=BlahThat.;TrustServerCertificate=yes;DATABASE=LightweightExample" --make-aliases --database LightweightExample --schema dbo --output ./src/examples/test_chinook/entities
```
