...
class SQLiteQueryFormatter: public SqlQueryFormatter
{
  ...
  public:
                                                               ...
                                                               SqlForeignKeyReferenceDefinition const& referencedColumn)
    {
        ...
        // emitted by `DropForeignKey`). The name is shared with the runtime rebuild
        // path (`SqliteRebuildAddForeignKey`) via `BuildForeignKeyConstraintName` so
        // CREATE-table and ALTER-table produce identical constraint names.
