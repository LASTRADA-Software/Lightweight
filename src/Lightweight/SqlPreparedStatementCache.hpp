...
///
/// Preparing a statement costs a server-side parse on MS SQL Server and PostgreSQL. Neither driver pays
/// it inside @c SQLPrepare, which sends nothing: it rides along with the first execute of the freshly
/// prepared handle, and a matching deallocate follows when the handle is freed. This cache keeps the
/// @c SQLHSTMT handles of recently prepared queries alive, so a repeat of the same SQL text on the same
/// connection re-executes a prepared handle instead. Measured behind a 50 ms link, that is worth about
/// three round-trips per query on psqlODBC and one on the Microsoft driver.
///
...
class SqlPreparedStatementCache final
{
  public:
    ...
    struct Statistics
    {
        ...
        /// Prepare requests a statement served from the handle it was already holding, i.e. a repeat of
        /// the query text it last prepared. Like a hit these cost no @c SQLPrepare, but they never touch
        /// the pool: parking the handle only to look that same text back up would be pure overhead.
    ...
    /// @see Statistics::directReuses
    void RecordDirectReuse() noexcept
    {
        ++m_stats.directReuses;
    }
