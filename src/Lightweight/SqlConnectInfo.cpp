...
bool EnsureSqliteDatabaseFileExists(SqlConnectionString const& connectionString)
{
    ...
    auto const driverIsSqlite =
        std::ranges::search(driver, std::string_view { "sqlite" }, [](char a, char b) {
    ...
    if (database == ":memory:" || database.starts_with("file::memory:")
        || (database.starts_with("file:") && database.find("mode=memory") != std::string::npos))
        return true;
