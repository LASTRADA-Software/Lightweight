...
std::u8string ConvertWindows1252ToUtf8(std::string_view input)
{
    std::u8string output;
    ...
    for (char ch: input)
    {
        ...
        else
        {
            ...
            output += static_cast<char8_t>(0xC0 | (c >> 6));
            output += static_cast<char8_t>(0x80 | (c & 0x3F));
        }
    }
