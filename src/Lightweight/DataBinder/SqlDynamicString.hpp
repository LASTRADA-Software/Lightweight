...
struct std::formatter<SqlDynamicString<N, T>>: std::formatter<std::string>
{
    ...
    auto format(value_type const& text, format_context& ctx) const
    {
        if constexpr (std::same_as<T, wchar_t>)
            ...
            return std::formatter<std::string>::format(text.data(), ctx);
    }
};
...
struct SqlBasicStringOperations<SqlDynamicString<N, T>>
{
    ...
    // TODO: We could e.g. store an indicator in the SqlDynamicString to indicate that the value was trimmed
    //
    static void PostProcessOutputColumn(ValueType* result, SQLLEN indicator)
    {
        switch (indicator)
        {
            ...
            case SQL_NO_TOTAL:
                ...
                break;
            default: {
                auto const actualTotalCharCount = static_cast<std::size_t>(indicator) / sizeof(CharType);
                result->resize((std::min)(N, actualTotalCharCount));
                break;
            }
        }
    }
