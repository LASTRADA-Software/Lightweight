...
namespace SqlMigration
{
    ...
    ///
    /// @ingroup SqlMigration
    class MigrationException: public SqlException
    {
      public:
        ...
        LIGHTWEIGHT_API MigrationException(Operation operation,
    ...
    class MigrationManager
    {
      public:
        ...
        /// @brief Sets the default schema applied to migration connections.
        ///
        /// When non-empty, installs a chained `SqlConnection::SetPostConnectedHook`
        /// that executes the formatter's `SetDefaultSchemaStatement(schema)`
        /// (e.g. `SET search_path TO "<schema>", public` on PostgreSQL) for
        /// every new connection. SQL Server and SQLite have no portable
        /// session-level "default schema" command — for SQL Server, the
        /// connecting login's server-side `DEFAULT_SCHEMA` is what determines
        /// where unqualified DDL/DML lands.
        ///
        /// `schema` is validated against a `[A-Za-z0-9_]` whitelist; rejecting
        /// invalid input by throwing a `std::invalid_argument`. Passing an
        /// empty string clears any previously installed schema hook.
        ///
        /// Calling this **after** a connection has already been opened on the
        /// current thread does not retroactively apply the schema — the hook
        /// fires on subsequent `SqlConnection::Connect` calls. For dbtool and
        /// the GUI this is invoked before the first connection.
        ///
        /// @param schema The default schema to use. Empty disables.
        LIGHTWEIGHT_API void SetDefaultSchema(std::string schema);

        /// @brief Returns the default schema currently installed on the manager
        /// (empty when unset).
        [[nodiscard]] LIGHTWEIGHT_API std::string_view DefaultSchema() const noexcept;
...
/// via @ref LIGHTWEIGHT_SQL_MIGRATION_DOWN.
///
...
/// @brief Companion to @ref LIGHTWEIGHT_SQL_MIGRATION_REVERSIBLE — defines the out-of-line `Down()` body.
///
