...
/// receives run directly on the thread that pumps it (its @c Run / @c Drain / @c RunOne / @c RunUntil
/// members), never on stdexec. (Blocking work is offloaded elsewhere, to a @ref ThreadPoolExecutor.)
/// That pump still needs a tiny thread-safe queue a single owning thread can block on
/// (@ref WaitAndPop), step (@ref TryPop), and be woken from (@ref Wake) when its stop flag flips.
/// stdexec's @c run_loop cannot express the predicate-gated @c RunUntil pump that @c SyncWaitPumping
/// relies on, so this stays hand-rolled.
///
