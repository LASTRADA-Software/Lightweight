...
class SqlConnection final
{
  public:
    ...
    /// @brief Server-type overload of @ref SupportsPreparedStatementReuse, for callers that hold only
    ///        the server type rather than a live connection.
    ///
    /// @param serverType The backend to answer for.
    /// @return `true` if prepared handles may be pooled and re-executed on @p serverType.
    [[nodiscard]] static bool SupportsPreparedStatementReuse(SqlServerType serverType) noexcept;
...
inline bool SqlConnection::SupportsPreparedStatementReuse(SqlServerType serverType) noexcept
{
...
{
    return SupportsPreparedStatementReuse(ServerType());
}
