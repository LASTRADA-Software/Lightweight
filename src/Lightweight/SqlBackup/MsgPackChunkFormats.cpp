...
namespace
{
    ...
    class MsgPackReaderBase: public ChunkReader
    {
      public:
        MsgPackReaderBase(uint8_t const* begin, uint8_t const* end):
            cursor_(begin),
            end_(end)
        {
        }
      ...
      protected:
        void SetRange(uint8_t const* begin, uint8_t const* end)
        {
            cursor_ = begin;
            end_ = end;
        }
    ...
    class MsgPackChunkReader: public MsgPackReaderBase
    {
      public:
        explicit MsgPackChunkReader(std::istream& input):
            MsgPackReaderBase(nullptr, nullptr)
        {
            ...
            SetRange(buffer_.data(), buffer_.data() + buffer_.size());
        }
    ...
    class MsgPackChunkReaderFromBuffer: public MsgPackReaderBase
    {
      public:
        explicit MsgPackChunkReaderFromBuffer(std::span<uint8_t const> buffer):
            MsgPackReaderBase(buffer.data(), buffer.data() + buffer.size())
        {
        }
    };
