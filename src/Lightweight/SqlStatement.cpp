...
bool SqlStatement::ReleasePreparedHandle() noexcept
{
    ...
    return cache != nullptr && ReleasePreparedHandle(*cache);
}

bool SqlStatement::ReleasePreparedHandle(SqlPreparedStatementCache& cache) noexcept
{
    if (m_hStmt == SQL_NULL_HSTMT || m_preparedQuery.empty())
        return false;
...
bool SqlStatement::AcquirePreparedHandle(SqlPreparedStatementCache& cache, std::string_view query)
{
...
bool SqlStatement::RetryStalePreparedStatement(SQLRETURN result)
{
    ...
    // left none on the handle; reading the handle regardless would match no stale-plan state and
    // return before re-preparing, defeating the seam. See EffectiveError().
...
std::optional<SqlGuid> RowArrayCursor::GetGuid(std::size_t rowInBatch, SQLUSMALLINT column) const
{
    ...
    if (!cell)
        return std::nullopt;
    // The cell holds the driver-filled SQLGUID in wire order; see SwapGuidWireByteOrder().
    SqlGuid value;
