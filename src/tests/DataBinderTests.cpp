...
TEST_CASE_METHOD(SqlTestFixture, "SqlFixedString: TrimRight", "[SqlFixedString]")
{
    ...
    REQUIRE(str == "Hello");
    SqlWideTrimmedFixedString<20> wstr { L"Hello        " };
    SqlBasicStringOperations<SqlWideTrimmedFixedString<20>>::TrimRight(&wstr, 10);
    REQUIRE(wstr == L"Hello");
    SqlWideTrimmedFixedString<20> wstrWrongIndicator { L"Hello        " };
...
template <>
struct TestTypeTraits<SqlWideTrimmedFixedString<20>>
{
    using ValueType = SqlWideTrimmedFixedString<20>;
