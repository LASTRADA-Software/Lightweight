...
TEST_CASE_METHOD(SqlTestFixture, "CustomBinding", "[DataMapper]")
{
    auto dm = DataMapper {};
    ...
    {
        auto a = CustomBindingA { .number = i,
                                  .name = std::format("Name-{}", i),
                                  .description = ToStdWideString(std::format("Description-{}", i)) };
        dm.Create(a);

        auto c = CustomBindingC { .value = i * 1.5, .comment = std::format("Comment-{}", i) };
        dm.Create(c);

        auto b = CustomBindingB { .title = std::format("Title-{}", i), .date_time = SqlDateTime::Now(), .a_id = a.id.Value(), .c_id = c.id.Value() };
        dm.Create(b);
    }
