...
// number of driver round-trips the cache statistics report. Installs itself as the active logger for
// its lifetime, restoring the previous one on scope exit (see ScopedSqlNullLogger).
struct PrepareProbe: ScopedSqlNullLogger
{
...
TEST_CASE_METHOD(SqlTestFixture, "PreparedStatementCache: repeated prepare hits the cache", "[SqlPreparedStatementCache]")
{
    ...
    // One statement repeating one query text is served from the handle it already holds, so the four
    // repeats never reach the pool - they are counted as direct reuses rather than as hits.
    auto const& stats = connection.PreparedStatementCache().Stats();
    CHECK(probe.prepares == 5); // five logical prepares ...
    CHECK(stats.misses == 1);   // ... but only the first one reached the driver
    CHECK(stats.directReuses == 4);
    CHECK(stats.hits == 0);
                 ...
                 "[SqlPreparedStatementCache]")
{
...

    {
        ...
        // sentinel must not reach the pool: a cache hit adopts the pooled count verbatim (no
        // SQLNumParams round-trip), and Execute() would then reject a perfectly valid argument list.
        // Releasing therefore pools the count SQLPrepare reported, not the sentinel.
        auto stmt = SqlStatement { connection };
        stmt.Prepare("SELECT value FROM stmt_cache WHERE id = ?");
        auto const id = 1;
                 ...
                 "[SqlPreparedStatementCache]")
{
...

    {
        ...
        // One statement, one query text: every repeat is served from the handle it already holds, so
        // the reuse shows up as a direct reuse and only the very first prepare reached the driver.
        auto const& stats = connection.PreparedStatementCache().Stats();
        CHECK(stats.directReuses > 0);
