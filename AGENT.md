...
| `src/Lightweight/Async/`                  | C++23 coroutine layer: `Task`, `SyncWait`, executors (thread-pool/strand/manual), offload backend, `AsyncSqlTransaction`. Async methods are added to `SqlConnection`/`DataMapper`/`Pool`. Gated by `LIGHTWEIGHT_ENABLE_ASYNC` (default ON). See `docs/async.md` |
