...
Lightweight supports both low-level access to the SQL API as well as provides hight level abstraction that allows easy database access.

Here you can see an example of datamapper usage (our tool for the high level abstraction)

## Supported platforms

For now, only ODBC is supported, so it should work on any platform that has an ODBC driver and
...
Example of its usage to create/read/update/delete entry in the database for one table
```cpp
...
In the presented example we use column aliasing. For more details see how-to\#rename-column-name page.
...
auto user_name = email.user->name; // lazily load the user record
```

### Mapping query results to a simple struct

If you have a SQL query that returns some values, but it does not correspond to the existing table in the database, you can map the result to a simple struct.
